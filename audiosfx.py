...
def conv_reverb(filename, ir="default_reverb_IR.wav", wet=0.2, wout=True):
    print("test")
    start = time.time()
    n, data, data_dB, sr, ch = inputwav(filename)
...
def compress(
    filename, threshold, ratio, makeup, attack, release, wout=True, plot=False
):
    """
    ...
    if plot == True:
        ...
        ax1.plot(t, dataCs_bit, "m-", linewidth=0.1, label=filename + " compressed")
        ax1.axhline(10 ** (threshold / 20.0), linestyle="-", color="cyan", linewidth=1)
        ax1.axhline(
            -(10 ** (threshold / 20.0)), linestyle="-", color="cyan", linewidth=1
        )
        ax1.legend()
        ax2.legend()
...
def gate(filename, threshold, ratio, attack, release, wout=True, plot=True):
    ...
    if wout == True:
        print("Exporting...")
        sf.write(
            filename[0 : len(filename) - 4] + "_gated.wav", dataCs_bit, sr, "PCM_16"
        )
    end = time.time()
    elapsed = int(1000 * (end - start))
    print("Done!")
...
def limit(filename, threshold, makeup, wout=True, plot=False):
    ...
    dataL, dataL_bit = compress(
        filename, threshold, 1000.0, makeup, 1.0, 500.0, wout=False, plot=plot
    )
    if wout == True:
        print("Exporting...")
        sf.write(
