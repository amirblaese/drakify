...
def distort(filename,threshold,type='smag',wout=True,plot=True):
    """
    ...
    threshold : scalar (bit)
        value in bits of the threshold above which signal is distorted
       
    type: string, optional, default=cubic
        Type of distortion that is applied. Default is cubic. Other options:
        'smag', 'power', 'digital', 'flat'
    ...
    if type=='smag':
        print('Applying smooth algorithm distortion...')
        a=1.0215#(np.exp(-1/(44.1*1)))
        r=1/1.0215#np.exp(-1/(44.1*1.1))
        for k in range(ch):
            for i in range (1,len(data)):
                    if data[i,k]>threshold:
                        if data[i-1,k]>=data[i,k]:
                            dataD[i,k]=a*dataD[i-1,k]+(1-a)*dataD[i,k]
                        if data[i-1,k]<data[i,k]:
                            dataD[i,k]=r*dataD[i-1,k]+(1-r)*dataD[i,k]
                    if data[i,k]<-threshold:
                        if data[i-1,k]<=data[i,k]:
                            dataD[i,k]=(a*dataD[i-1,k]+(1-a)*dataD[i,k])
                        if data[i-1,k]>data[i,k]:
                            dataD[i,k]=(r*dataD[i-1,k]+(1-r)*dataD[i,k])
    if type=='power':
        print('Applying power distortion...')
        for k in range(ch):
            for i in range (0,len(data)):
                if dataD[i,k]>threshold:
                    dataD[i,k]=(1+dataD[i,k])**(0.88)-1
                if dataD[i,k]<-threshold:
                    dataD[i,k]=-1*(abs(dataD[i,k]-1))**(0.88)+1
    if type=='flat':
        print('Applying flat distortion...')
        for k in range(ch):
            for i in range (0,len(data)):
                if dataD[i,k]>threshold:
                    dataD[i,k]=(0.8*dataD[i,k])
                if dataD[i,k]<-threshold:
                    dataD[i,k]=(0.8*dataD[i,k])
    if type=='digital':
        print('Applying digital distortion...')
        for k in range(ch):
            for i in range (0,len(data)):
                if dataD[i,k]>threshold:
                    dataD[i,k]=threshold
                if dataD[i,k]<-threshold:
                    dataD[i,k]=-threshold
    if type=='cubic':
        print('Applying cubic distortion...')
        for k in range(ch):
            for i in range (0,len(data)):
                if dataD[i,k]>threshold:
